...
const CONTROL_HUB_API =
  process.env.CONTROL_HUB_API || process.env.CONTROL_HUB_URL;
...
export async function sendOrderToControlHub(orderData, paymentData = null) {
  try {
    ...
    const controlHubOrder = {
      ...
      shippingAddress:
        orderData.shippingAddress || orderData.billingDetails?.address || {},
      billingAddress:
        orderData.billingDetails?.address || orderData.shippingAddress || {},
    ...
    if (!response.ok) {
      throw new Error(
        `Control Hub API error: ${response.status} ${response.statusText}`,
      );
    }
...
function determineOrderType(orderData) {
  ...
  const hasCustomDesign = items.some(
    (item) => item.customDesign || item.uploadedArt,
  );
  const isWholesale =
    orderData.wholesale ||
