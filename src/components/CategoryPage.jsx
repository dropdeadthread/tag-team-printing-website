...
const CategoryPage = ({ categorySlug: propCategorySlug, categoryName: propCategoryName, categoryId: propCategoryId }) => {
  const routeParams = useParams();
  
  const categorySlug = propCategorySlug || routeParams?.categorySlug;
  const categoryName = propCategoryName || (categorySlug ? categorySlug.replace(/-/g, " ") : "");
  ...
  const content = (
    <>
      ...
      {!loading && !error && !products.length && <p>No products found in this category.</p>}
        ...
        <>
          ...
          <ul style={{ display: "flex", flexWrap: "wrap", gap: "2rem", listStyle: "none", padding: 0 }}>
            {products.map(prod => {
              ...
              console.log('Creating link for:', { name, styleID, styleName });
              
              return (
                <li key={styleID || Math.random()} style={{ width: 220, textAlign: "center" }}>
                  <Link 
                  ...
                  >
                    <img 
                      ...
                      style={{ width: 200, height: 200, objectFit: "contain", background: "#f8f8f8", borderRadius: 8 }}
                      loading="lazy"
                    />
                    <div style={{ marginTop: 8, fontWeight: "bold", color: "#fff5d1" }}>{name}</div>
                    <div style={{ fontSize: 14, color: "#ccc" }}>{brand}</div>
                    <div style={{ fontSize: 12, color: "#999" }}>Style: {styleName}</div>
                  </Link>
                </li>
              );
            })}
