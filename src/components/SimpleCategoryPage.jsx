...
const SimpleCategoryPage = ({ categoryId, categoryName, categorySlug }) => {
  ...
  const getAvailableBrands = () => {
    ...
    const productsToCheck = allCategoryProducts.length > 0 ? allCategoryProducts : products;
    
    if (productsToCheck.length === 0) {
      // Return default brands while products are loading
      return ['Gildan', 'JERZEES', 'BELLA + CANVAS', 'Next Level', 'Comfort Colors', 'Threadfast Apparel'];
    }
    
    // Get unique brands from all category products
    const brandsInCategory = [...new Set(productsToCheck.map(product => 
  ...
  const searchByBrand = async (brandName) => {
    ...
    try {
      console.log("Searching for brand:", brandName, "in category:", categoryId);
      
      // Filter from all category products, not just current page
      const productsToSearch = allCategoryProducts.length > 0 ? allCategoryProducts : products;
      const brandResults = productsToSearch.filter(product => 
        (product.brandName === brandName || product.brand === brandName)
      );
      
      console.log("Brand search results for", brandName, ":", brandResults.length, "products");
  ...
  useEffect(() => {
    console.log("Fetching products for category:", categoryId, "page:", currentPage);
    setLoading(true);
    
    // First, get all products in category for brand filtering (without pagination)
    fetch(`/api/list-products?category=${categoryId}&limit=1000`) // Get all products
      .then(response => response.json())
      .then(data => {
        console.log("All category products loaded:", data.products?.length || 0);
    ...
    fetch(`/api/list-products?category=${categoryId}&page=${currentPage}&limit=20`)
      .then(response => {
        if (!response.ok) {
      ...
      .then(data => {
        ...
        setPagination({
          ...
          totalProducts: data.pagination?.totalProducts || (data.products || data).length,
    ...
    if (showWomensOnly) {
      filtered = filtered.filter(product => {
        ...
        return title.includes('women') || 
               ...
               (styleCode.includes('w') && !styleCode.includes('sw')); // W but not SW (southwest, etc)
      });
    }
    ...
    if (priceRange && priceRange !== '') {
      ...
      filtered.sort((a, b) => {
        switch (sortBy) {
          ...
          case 'brand':
            return (a.brandName || a.brand || '').localeCompare(b.brandName || b.brand || '');
          case 'price-low':
  ...
  const getDisplayPagination = () => {
    const hasActiveFilters = showWomensOnly || selectedBrands.length > 0 || priceRange !== 'all';
    ...
    if (hasActiveFilters) {
      ...
      const totalFilteredPages = Math.max(1, Math.ceil(totalFilteredProducts / productsPerPage));
  ...
  const getWomensCount = () => {
    ...
    return productsToCheck.filter(product => {
      ...
      return title.includes('women') || 
             ...
             description.includes('ladies') ||
             (styleCode.endsWith('l') && title.includes('women')) || // L suffix + women in title
             (styleCode.includes('w') && !styleCode.includes('sw')); // W but not SW
    }).length;
  };
      ...
      <div style={{
          ...
          <>
            ...
            <div style={{
              ...
              <div style={{
                ...
                {!priceRange && (
                  <div>
                    <label htmlFor="sort-by-select" style={{
                      display: 'block',
                      marginBottom: '0.5rem',
                      color: '#fff5d1',
                      fontWeight: '500',
                ...
                {/* Brand Dropdown */}
                <div>
                  <label htmlFor="brand-select" style={{
                    display: 'block',
                    marginBottom: '0.5rem',
                    color: '#fff5d1',
                    fontWeight: '500',
              ...
              {(selectedBrands.length > 0 || priceRange || searchQuery || (showWomensOnly && categoryId !== 11 && categorySlug !== 'headwear')) && (
                <div style={{
                  marginBottom: '1rem',
                  padding: '1rem',
                  ...
                  <div style={{
                    ...
                    {showWomensOnly && categoryId !== 11 && categorySlug !== 'headwear' && (
                      <span
                        style={{
                          padding: '0.25rem 0.5rem',
                          backgroundColor: '#28a745',
                ...
                {availableBrands.map(brand => (
                  <button
                    ...
                    style={{
                      padding: '0.5rem 1rem',
                      borderRadius: '6px',
                      backgroundColor: searchQuery === brand ? '#28a745' : '#007bff',
                      color: '#fff',
                      border: 'none',
                  ...
                  >
                    {searchLoading && searchQuery === brand ? 'Searching...' : brand}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="category-product-grid">
              {(searchQuery ? searchResults : displayedProducts).map((prod, index) => {
                const name = prod.title || prod.name || prod.styleName || "Unknown Product";
                const brand = prod.brandName || prod.brand || "Unknown Brand";
                const styleID = prod.styleID || "";
                const styleName = prod.styleName || prod.styleCode || prod.styleID || "unknown";
                ...
                console.log(`Product ${index}:`, { name, brand, styleID, styleName, imageUrl });
        ...
        {displayPagination.totalPages > 1 && (
          <div style={{
            ...
            <button
              onClick={() => setCurrentPage(prev => Math.min(displayPagination.totalPages, prev + 1))}
              disabled={currentPage === displayPagination.totalPages}
              style={{
                padding: '0.5rem 1rem',
                background: currentPage === displayPagination.totalPages ? '#666' : '#fff5d1',
                color: currentPage === displayPagination.totalPages ? '#999' : '#000',
                border: 'none',
                borderRadius: '4px',
                cursor: currentPage === displayPagination.totalPages ? 'not-allowed' : 'pointer',
                fontWeight: 'bold'
              }}
            >
              Next
        ...
        {/* Debug Info */}
        <details style={{ marginTop: '3rem', fontSize: '0.8rem', opacity: 0.7 }}>
          <summary style={{ color: '#fff5d1', cursor: 'pointer' }}>Debug Info</summary>
          <pre style={{ background: 'rgba(0, 0, 0, 0.9)', color: '#fff5d1', padding: '1rem', borderRadius: '4px', overflow: 'auto', border: '1px solid #fff5d1' }}>
