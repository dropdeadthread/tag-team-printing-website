...
const SimpleProductPageTemplate = ({ pageContext }) => {
    ...
    const loadProduct = async () => {
      try {
        ...
        if (!Array.isArray(allProducts)) {
          console.error('Expected allProducts to be an array, got:', typeof allProducts, allProducts);
          allProducts = [];
        }
        
        const foundProduct = allProducts.find(p => p.styleName === pageContext.styleCode);
        ...
        if (foundProduct?.styleID) {
          const inventoryResponse = await fetch(`/api/get-inventory?styleID=${foundProduct.styleID}`);
          const inventory = await inventoryResponse.json();
          setInventoryData(inventory);
          
          // Set default selected size to first available
          const sizesWithStock = Object.keys(inventory.sizes || {}).filter(size => 
            inventory.sizes[size].available > 0
          );
      ...
      <Layout>
        <div style={{ 
          ...
          <div style={{ textAlign: 'center' }}>
            <h1 style={{ color: '#333', marginBottom: '1rem' }}>Loading product...</h1>
            <div style={{ 
              width: '40px', 
              height: '40px', 
    ...
    return (
      <Layout>
        <div style={{ 
          ...
          <p style={{ color: '#666' }}>Looking for style code: {pageContext.styleCode}</p>
        </div>
      </Layout>
    );
  }

  // Use dynamic images based on product and color
  const getProductImageUrl = (product, selectedColor) => {
    if (!product?.styleID) return '/images/placeholder.png';
    
    // For development, you can temporarily use placeholder images
    // Comment out the next line to use S&S images in development
    // if (process.env.NODE_ENV === 'development') return '/images/placeholder.png';
  ...
  const currentPrice = inventoryData?.sizes?.[selectedSize]?.price || 25.00;
  ...
  const handleAddToCart = () => {
    ...
    alert(`Added ${quantity} x ${product.title} (${selectedSize}${selectedColor ? `, ${selectedColor.name}` : ''}) to cart!`);
  };
  ...
  const handleColorChange = async (color) => {
    ...
    if (product?.styleID && color) {
      try {
        ...
        const inventoryResponse = await fetch(`/api/get-inventory?styleID=${product.styleID}&color=${encodeURIComponent(color.name)}`);
        ...
        const availableSizes = Object.keys(colorInventory.sizes || {}).filter(size => 
          colorInventory.sizes[size].available > 0
        );
          ...
          <div style={{ 
            ...
            <div style={{ position: 'relative' }}>
              ...
              <div style={{ 
                marginTop: '1.5rem',
                padding: '2rem',
                background: 'linear-gradient(135deg, rgba(255, 80, 80, 0.15) 0%, rgba(204, 64, 64, 0.15) 100%)',
                ...
                <div style={{ position: 'relative', zIndex: 1 }}>
                  ...
                  <div style={{
                    ...
                    }}>✓ Championship quality prints</span>
                    <span style={{
                      color: '#fff',
                      fontSize: '0.9rem',
                      padding: '0.3rem 0.8rem',
                      ...
                      borderRadius: '15px',
                      border: '1px solid rgba(255, 245, 209, 0.3)'
                    }}>✓ Fast turnaround, no job too tough</span>
                    <span style={{
                      color: '#fff',
                      fontSize: '0.9rem',
                      padding: '0.3rem 0.8rem',
                      ...
                      borderRadius: '15px',
                      border: '1px solid rgba(255, 245, 209, 0.3)'
                    }}>✓ Our team brings your vision to the main event</span>
                  </div>
                  ...
                  <button
                    ...
                    onMouseOut={(e) => {
                      e.target.style.transform = showPrintOrder ? 'scale(0.98)' : 'scale(1)';
                      e.target.style.boxShadow = showPrintOrder 
                        ? '0 8px 20px rgba(40, 167, 69, 0.4)'
                        : '0 8px 20px rgba(255,80,80,0.5)';
                    }}
              ...
              {showPrintOrder && (
                <div style={{
                  ...
                  borderRadius: '16px',
                  boxShadow: '0 12px 30px rgba(0, 0, 0, 0.5), 5px 5px 0 #ff5050',
                  ...
                  <div style={{
                    background: 'linear-gradient(135deg, rgba(255, 245, 209, 0.15) 0%, rgba(255, 245, 209, 0.08) 100%)',
                    padding: '1.5rem',
                    borderRadius: '12px',
                    ...
                    <div style={{
                      ...
                      <div>
                        ...
                        <p style={{ 
                          ...
                          Size: <strong>{selectedSize}</strong> • Color: <strong>{selectedColor?.name || 'Standard'}</strong>
                        </p>
                        <p style={{ 
                          color: '#fff5d1', 
                          margin: '0.25rem 0 0 0', 
                          ...
                          Garment Price: <span style={{ color: '#ff5050' }}>${currentPrice.toFixed(2)} each</span>
                        </p>
                      </div>
                    </div>
                    ...
                    <div style={{
                      ...
                      <p style={{ 
                        ...
                        <br />
                        <span style={{ color: 'rgba(255, 245, 209, 0.9)', fontSize: '0.85rem' }}>
                          Work with our in-house designers to create bold, custom art that leaves a mark.
                        </span>
                      </p>
                    </div>
                  </div>
                  ...
                  <IntegratedPrintOrderForm 
                    ...
                    preSelectedGarment={{
                      title: product.title,
                      price: currentPrice,
                      wholesalePrice: inventoryData?.sizes?.[selectedSize]?.wholesalePrice || null,
            ...
            <div style={{ marginLeft: '4rem' }}>
              ...
              {showProductDetails && (
                <div style={{ 
                  ...
                  <div style={{ marginBottom: '0.5rem' }}>
                    ...
                    <span style={{ color: '#666', marginLeft: '0.5rem' }}>{product.brandName}</span>
                  </div>
                  <div style={{ marginBottom: '0.5rem' }}>
                    <strong style={{ color: '#333' }}>Style:</strong> 
                    <span style={{ color: '#666', marginLeft: '0.5rem' }}>{product.styleName}</span>
                  </div>
                  <div style={{ marginBottom: '0.5rem' }}>
                    <strong style={{ color: '#333' }}>Category:</strong> 
                    <span style={{ color: '#666', marginLeft: '0.5rem' }}>{product.baseCategory}</span>
                  </div>
                  <div>
                    <strong style={{ color: '#333' }}>Part Number:</strong> 
                    <span style={{ color: '#666', marginLeft: '0.5rem' }}>{product.partNumber}</span>
                  </div>
                  {product.description && (
                    <>
                      <hr style={{ margin: '1rem 0', border: 'none', borderTop: '1px solid #ddd' }} />
                      <div>
                        <strong style={{ color: '#333', display: 'block', marginBottom: '0.5rem' }}>Description:</strong>
                        <div 
                ...
                <div style={{
                  ...
                  <div style={{ overflowX: 'auto' }}>
                    <table style={{
                      ...
                      <tbody>
                        {inventoryData?.sizes && Object.keys(inventoryData.sizes).map((size) => {
                          ...
                          const measurements = {
                            ...
                            'XXL': { chest: '50-53', length: '32', sleeve: '11' },
                            '2XL': { chest: '50-53', length: '32', sleeve: '11' },
                            'XXXL': { chest: '54-57', length: '33', sleeve: '11.5' },
                            '3XL': { chest: '54-57', length: '33', sleeve: '11.5' },
                            '4XL': { chest: '58-61', length: '34', sleeve: '12' },
                            '5XL': { chest: '62-65', length: '35', sleeve: '12.5' }
                          };
                          const sizeData = measurements[size] || { chest: 'N/A', length: 'N/A', sleeve: 'N/A' };
                          
                          return (
                            <tr key={size} style={{
                              backgroundColor: selectedSize === size ? '#e7f1ff' : 'white'
                            }}>
                              <td style={{ 
                                padding: '0.75rem', 
                                border: '1px solid #dee2e6',
                                fontWeight: selectedSize === size ? 'bold' : 'normal',
                                color: selectedSize === size ? '#007bff' : '#495057'
                              }}>
                    ...
                    <div style={{
                      ...
                      <h4 style={{ margin: '0 0 0.5rem 0', color: '#2e7d32', fontSize: '1rem' }}>
                        📐 How to Measure
                      </h4>
                      <div style={{ fontSize: '0.85rem', color: '#333', lineHeight: '1.6' }}>
                        <div><strong>Chest:</strong> Measure around the fullest part of your chest, keeping the tape horizontal.</div>
                        <div><strong>Length:</strong> Measure from the highest point of the shoulder to the bottom hem.</div>
                        <div><strong>Sleeve:</strong> Measure from the center back neck to the end of the sleeve.</div>
                      </div>
                    </div>
                    ...
                    <div style={{
                      ...
                      <h4 style={{ margin: '0 0 0.5rem 0', color: '#856404', fontSize: '1rem' }}>
                        ℹ️ Size Guide Notes
                      </h4>
                      <div style={{ fontSize: '0.85rem', color: '#333', lineHeight: '1.6' }}>
                        <div>• Measurements are approximate and may vary by ±1 inch</div>
                        <div>• For between sizes, we recommend sizing up for comfort</div>
                        <div>• Contact us for specific brand sizing questions</div>
                        {product?.sustainableStyle && (
                          <div style={{ color: '#388e3c', fontWeight: 'bold', marginTop: '0.5rem' }}>
                ...
                <div style={{ marginBottom: '1.5rem' }}>
                    ...
                    {Object.entries(inventoryData.sizes).map(([size, data]) => (
                      <button
                        ...
                        style={{
                          ...
                          fontSize: '1rem',
                          borderRadius: '8px',
                          border: selectedSize === size ? '2px solid #ff5050' : '2px solid #ddd',
                          background: data.available === 0 ? '#f8f9fa' : (selectedSize === size ? '#ff5050' : '#fff'),
                          color: data.available === 0 ? '#999' : (selectedSize === size ? '#fff' : '#333'),
                          cursor: data.available === 0 ? 'not-allowed' : 'pointer',
                          fontWeight: selectedSize === size ? 'bold' : 'normal',
                      ...
                      >
                        ...
                        <small style={{ fontSize: '0.8rem' }}>
                          {data.available === 0 ? 'Out' : `${data.available} left`}
                        </small>
                      </button>
                    ))}
                  </div>
                  <div style={{ 
                    ...
                    {stockAmount === 0 ? 'Out of Stock' : 
                     stockAmount < 5 ? `Only ${stockAmount} left in stock!` :
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      fontSize: '1.2rem',
                      borderRadius: '4px',
                      cursor: quantity >= stockAmount ? 'not-allowed' : 'pointer',
                      fontWeight: 'bold'
                    }}
                  >
                    +
              ...
              <button
                ...
                style={{
                  background: addedToCart ? 'linear-gradient(135deg, #28a745 0%, #20c997 100%)' : 
                            (stockAmount === 0 ? '#ccc' : '#ff5050'),
                  color: 'white',
                  border: 'none',
                  padding: '1rem 2rem',
                  fontSize: '1.2rem',
                  ...
                  fontWeight: 'bold',
                  transition: 'all 0.3s ease',
                  boxShadow: addedToCart ? '0 6px 12px rgba(40, 167, 69, 0.4)' : '0 6px 12px rgba(0,0,0,0.4)',
                  width: '100%',
                ...
                <div style={{ marginBottom: '1.5rem' }}>
                  ...
                  <div style={{ 
                    ...
                    {inventoryData.colors.map((color, index) => {
                      ...
                      const colorStock = selectedColor?.name === color.name && inventoryData?.sizes
                        ? Object.values(inventoryData.sizes).reduce((total, size) => total + (size.available || 0), 0)
                        : null;
                      
                      return (
                        <div 
                          key={index}
                          onClick={() => color.available && !colorInventoryLoading && handleColorChange(color)}
                          onKeyDown={(e) => {
                            if ((e.key === 'Enter' || e.key === ' ') && color.available && !colorInventoryLoading) {
                              e.preventDefault();
                              handleColorChange(color);
                            }
                          }}
                          tabIndex={color.available && !colorInventoryLoading ? 0 : -1}
                          role="button"
                          ...
                          style={{
                            ...
                            alignItems: 'center',
                            gap: '0.25rem',
                            cursor: color.available && !colorInventoryLoading ? 'pointer' : 'not-allowed'
                          }}
                        >
                          <div
                            style={{
                              ...
                              border: selectedColor?.name === color.name ? '4px solid #ff5050' : '3px solid #ddd',
                              opacity: color.available && !colorInventoryLoading ? 1 : 0.5,
                              transition: 'all 0.2s ease',
                              position: 'relative',
                              display: 'flex',
                              alignItems: 'center',
                              ...
                              boxShadow: selectedColor?.name === color.name ? '0 4px 12px rgba(255, 80, 80, 0.5)' : '0 2px 4px rgba(0,0,0,0.3)'
                            }}
                          ...
                          >
                            ...
                            {colorInventoryLoading && selectedColor?.name === color.name && (
                              <div style={{
                                position: 'absolute',
                                width: '20px',
                                height: '20px',
                          ...
                          <span style={{
                            ...
                            {color.name}
                            {colorStock !== null && selectedColor?.name === color.name && (
                              <div style={{ fontSize: '0.7rem', color: '#ffcc00' }}>
                                {colorStock} in stock
                              </div>
                            )}
                          </span>
