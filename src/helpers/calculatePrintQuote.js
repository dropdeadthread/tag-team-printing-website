...
export function calculatePrintQuote({
  ...
  const isDarkGarment = needsUnderbase !== null ? needsUnderbase : !lightColors.includes(garmentColor.toLowerCase());
  const isPolyester = polyesterPercent >= 50;
  
  // Check if only white ink is being used
  const isOnlyWhiteInk = inkColors.length > 0 && inkColors.every(color => 
  ...
  if (needsUnderbase !== null && !(isOnlyWhiteInk && colorCount === 1 && isDarkGarment)) {
  ...
  const setupTotal = totalScreens * locationCount * PRICING_CONFIG.PRINT_SETUP_FEE_PER_COLOR;
  ...
  const firstColorCharge = finalNeedsUnderbase ? PRICING_CONFIG.PRINT_FIRST_COLOR_WITH_UNDERBASE : PRICING_CONFIG.PRINT_FIRST_COLOR_NO_UNDERBASE;
  const additionalColorCharge = colorCount > 1
    ? (colorCount - 1) * (isPremiumInk ? PRICING_CONFIG.PRINT_ADDITIONAL_COLOR_PREMIUM : PRICING_CONFIG.PRINT_ADDITIONAL_COLOR_STANDARD)
    : 0;

  // Multiply print charges by location count
  const totalColorCharges = (firstColorCharge + additionalColorCharge) * locationCount;
