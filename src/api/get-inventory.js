import { calculateRetailPrice, getSizeAdjustedRetailPrice, getSizeAdjustedWholesalePrice, sortSizesByOrder, getQuantityBasedPrice } from '../config/pricing.js';
...
export default async function handler(req, res) {
  try {
    ...
    console.log(`Getting inventory for styleID: ${styleID}${color ? ` with color: ${color}` : ''}${quantity ? ` with quantity: ${quantity}` : ''}`);
    ...
    if (!username || !apiKey) {
      console.warn('SSActivewear API credentials not found, using realistic fallback data');
      return res.status(200).json(getRealisticInventory(styleID, color, quantity));
    }
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(`SSActivewear API error: ${response.status}, falling back to realistic data`);
      ...
      if (!Array.isArray(products) || products.length === 0) {
        console.warn('No products found in SSActivewear API, using realistic fallback data');
        return res.status(200).json(getRealisticInventory(styleID, color));
      }
      ...
      products.forEach(product => {
        ...
        if (product.sizeName && product.qty !== undefined) {
          ...
          if (!sizesMap[sizeKey]) {
            ...
            const baseWholesalePrice = parseFloat(product.customerPrice) || parseFloat(product.piecePrice) || 12.00; // Fallback wholesale
            const adjustedWholesalePrice = getSizeAdjustedWholesalePrice(baseWholesalePrice, sizeKey);
            const retailPrice = parseFloat(calculateRetailPrice(adjustedWholesalePrice));
      ...
      if (Object.keys(sizesMap).length > 0 && Object.keys(colorsMap).length > 0) {
        // Sort sizes in proper order (XS first, then progressive sizing)
        const sortedSizes = sortSizesByOrder(sizesMap);
...
function getProductInfo(styleID) {
  ...
  if (style.includes('bella') || style.includes('canvas') || style.includes('3001') || style.includes('3413')) {
    return {
      brand: 'Bella Canvas',
      wholesalePrice: 7.75,
      category: 'premium'
    };
  } else if (style.includes('next level') || style.includes('6210') || style.includes('6440')) {
    return {
      brand: 'Next Level',
      wholesalePrice: 6.50,
      category: 'premium'
    };
  } else if (style.includes('as colour') || style.includes('5001') || style.includes('5065')) {
    return {
      brand: 'AS Colour',
      wholesalePrice: 8.25,
      category: 'premium'
    };
  } else if (style.includes('hanes') || style.includes('5250') || style.includes('4980')) {
    return {
      brand: 'Hanes',
      wholesalePrice: 5.50,
      category: 'quality'
...
function getRealisticInventory(styleID, selectedColor = null, quantity = null) {
  ...
  if (selectedColor) {
    const colorMatch = realisticColors.find(c => 
      c.name.toLowerCase() === selectedColor.toLowerCase()
    );
    
    if (colorMatch) {
      // Generate color-specific inventory with slight variations
      const colorMultiplier = colorMatch.available ? (0.8 + Math.random() * 0.4) : 0;
      ...
      const sizes = {
        'XS': { 
          available: Math.floor((5 + Math.floor(Math.random() * 15)) * colorMultiplier), 
          price: parseFloat(getQuantityBasedPrice(getSizeAdjustedWholesalePrice(baseWholesalePrice, 'XS'), qty, productInfo.brand)), 
          wholesalePrice: getSizeAdjustedWholesalePrice(baseWholesalePrice, 'XS') 
        },
        'S': { 
          available: Math.floor((15 + Math.floor(Math.random() * 20)) * colorMultiplier), 
          price: parseFloat(getQuantityBasedPrice(getSizeAdjustedWholesalePrice(baseWholesalePrice, 'S'), qty, productInfo.brand)), 
          wholesalePrice: getSizeAdjustedWholesalePrice(baseWholesalePrice, 'S') 
        },
        'M': { 
          available: Math.floor((25 + Math.floor(Math.random() * 30)) * colorMultiplier), 
          price: parseFloat(getQuantityBasedPrice(getSizeAdjustedWholesalePrice(baseWholesalePrice, 'M'), qty, productInfo.brand)), 
          wholesalePrice: getSizeAdjustedWholesalePrice(baseWholesalePrice, 'M') 
        },
        'L': { 
          available: Math.floor((20 + Math.floor(Math.random() * 25)) * colorMultiplier), 
          price: parseFloat(getQuantityBasedPrice(getSizeAdjustedWholesalePrice(baseWholesalePrice, 'L'), qty, productInfo.brand)), 
          wholesalePrice: getSizeAdjustedWholesalePrice(baseWholesalePrice, 'L') 
        },
        'XL': { 
          available: Math.floor((18 + Math.floor(Math.random() * 22)) * colorMultiplier), 
          price: parseFloat(getQuantityBasedPrice(getSizeAdjustedWholesalePrice(baseWholesalePrice, 'XL'), qty, productInfo.brand)), 
          wholesalePrice: getSizeAdjustedWholesalePrice(baseWholesalePrice, 'XL') 
        },
        'XXL': { 
          available: Math.floor((8 + Math.floor(Math.random() * 15)) * colorMultiplier), 
          price: parseFloat(getQuantityBasedPrice(getSizeAdjustedWholesalePrice(baseWholesalePrice, 'XXL'), qty, productInfo.brand)), 
          wholesalePrice: getSizeAdjustedWholesalePrice(baseWholesalePrice, 'XXL') 
        },
        'XXXL': { 
          available: Math.floor((3 + Math.floor(Math.random() * 8)) * colorMultiplier), 
          price: parseFloat(getQuantityBasedPrice(getSizeAdjustedWholesalePrice(baseWholesalePrice, 'XXXL'), qty, productInfo.brand)), 
          wholesalePrice: getSizeAdjustedWholesalePrice(baseWholesalePrice, 'XXXL') 
        },
        '4XL': { 
          available: Math.floor((2 + Math.floor(Math.random() * 5)) * colorMultiplier), 
          price: parseFloat(getQuantityBasedPrice(getSizeAdjustedWholesalePrice(baseWholesalePrice, '4XL'), qty, productInfo.brand)), 
          wholesalePrice: getSizeAdjustedWholesalePrice(baseWholesalePrice, '4XL') 
        },
        '5XL': { 
          available: Math.floor((1 + Math.floor(Math.random() * 3)) * colorMultiplier), 
          price: parseFloat(getQuantityBasedPrice(getSizeAdjustedWholesalePrice(baseWholesalePrice, '5XL'), qty, productInfo.brand)), 
      ...
      const totalStock = Object.values(sortedSizes).reduce((sum, size) => sum + size.available, 0);
