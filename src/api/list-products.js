...
module.exports = async (req, res) => {
  try {
    ...
    console.log(`Brand distribution:`, brandFiltered.reduce((acc, item) => {
    ...
    const categoryProducts = brandFiltered.filter(item => {
      ...
      if (category.toString() === '21') {
        ...
        return (itemCategories.includes('21') || // Standard T-shirt category
      ...
      if (category.toString() === '64') {
        return itemCategories.includes('64') || // Tank tops category
      ...
      if (category.toString() === '8') {
        ...
        const bellaApprovedLongSleeve = ['3501Y', '3501', '3501CVC', '3501YCVC', '3511', '3512', '3513', '3513Y', '4651', '8850'];
        
        if (item.brandName === 'BELLA + CANVAS') {
          return bellaApprovedLongSleeve.includes(item.styleName) &&
                 // Exclude tank tops
                 !(item.title.toLowerCase().includes('tank')) &&
        ...
        return (item.baseCategory === 'T-Shirts - Long Sleeve' ||
      ...
      if (category.toString() === '400') {
        return (item.title.toLowerCase().includes('crewneck') ||
                item.title.toLowerCase().includes('crew neck') ||
                (item.baseCategory && (item.baseCategory.includes('Fleece - Premium - Crew') || 
                                     item.baseCategory.includes('Fleece - Core - Crew')))) &&
               // Exclude hooded items (those go to hoodies category)
               !(item.title.toLowerCase().includes('hoodie')) &&
               !(item.title.toLowerCase().includes('hooded')) &&
               !(item.title.toLowerCase().includes('hood')) &&
               !(item.baseCategory && (item.baseCategory.includes('Fleece - Premium - Hood') ||
      ...
      if (category.toString() === '401') {
        return item.brandName === 'BELLA + CANVAS' &&
               ((item.title.toLowerCase().includes('sponge fleece') && 
                 (item.title.toLowerCase().includes('hoodie') || item.title.toLowerCase().includes('hooded'))) ||
      ...
      if (category.toString() === '9') {
        return itemCategories.includes('9') || 
               (item.baseCategory && (item.baseCategory.includes('Fleece - Premium - Crew') ||
                                     item.baseCategory.includes('Fleece - Core - Crew'))) ||
               (item.title && (item.title.toLowerCase().includes('sweatshirt') ||
                              item.title.toLowerCase().includes('crewneck'))) &&
      ...
      if (category.toString() === '22' || category.toString() === '36') {
        return (itemCategories.includes('22') || itemCategories.includes('36') ||
               (item.baseCategory && (item.baseCategory.includes('Fleece - Premium - Hood') ||
                                     item.baseCategory.includes('Fleece - Core - Hood')))) &&
               (item.title && (item.title.toLowerCase().includes('hoodie') ||
                              item.title.toLowerCase().includes('hooded') ||
                              item.title.toLowerCase().includes('hood'))) &&
      ...
      if (category.toString() === '2') {
        return itemCategories.includes('2') && 
      ...
      if (category.toString() === '6') {
        ...
        return hatBrands.includes(item.brandName) && 
    ...
    console.log(`Filtered from ${brandFiltered.length} to ${categoryProducts.length} products for category ${category}`)
